  ...
  class QuickHull {
    ...
    computeExtremes() {
      ...
      this.tolerance =
        3 *
        Number.EPSILON *
        (Math.max(Math.abs(min[0]), Math.abs(max[0])) + Math.max(Math.abs(min[1]), Math.abs(max[1])) + Math.max(Math.abs(min[2]), Math.abs(max[2])))
      if (debug.enabled) {
    ...
    addAdjoiningFace(eyeVertex, horizonEdge) {
      ...
      //                        2
      //
      let face = Face.createTriangle(eyeVertex, horizonEdge.tail(), horizonEdge.head())
